...
- Search: `/`, `?`, `n`, `N`
- Command line: `:w`, `:w!`, `:wq`, `:q`, `:q!`, `:e!`; `:q` returns to preview without saving; `:<n>` (e.g. `:42`) jumps to line number `n`; `:set rnu` / `:set relativenumber` enables relative line numbers (persisted to config); `:set nornu` / `:set norelativenumber` disables them
