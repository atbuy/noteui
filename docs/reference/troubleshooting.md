...
- `config warning: sync profile "x": insecure_skip_tls_verify is enabled; TLS certificate verification is disabled ...`: this is an intentional reminder, not an error. TLS verification is off for that WebDAV profile, so the connection can be intercepted. Prefer trusting the server's CA with `ca_cert` and removing the flag; keep it only if you accept the risk on a trusted network. On an `http://` URL the flag does nothing and the warning says so.
