...
| `insecure_skip_tls_verify` | no | `false` | Disable TLS certificate verification. Use only on trusted internal networks where the server has a self-signed or IP-only certificate. noteui warns at startup while this is active on an `https://` profile, since the connection is then exposed to man-in-the-middle attacks; prefer `ca_cert`. |
